...
public class AdaptiveGridFTPClient2 {

    private static final Log LOG = LogFactory.getLog(AdaptiveGridFTPClient2.class);
    ...
    // dynamic values will store here
    // new data and current datasize in waiting list.
    private LinkedList<XferList.MlsxEntry> streamDataList= new LinkedList<>();
    private long lastDataSize;

    public AdaptiveGridFTPClient2() {
    ...
    @VisibleForTesting
    public AdaptiveGridFTPClient2(GridFTPTransfer gridFTPClient) {
    ...
    public static void main(String[] args) throws Exception {
        AdaptiveGridFTPClient2 multiChunk = new AdaptiveGridFTPClient2();
        multiChunk.parseArguments(args, multiChunk);

        Thread t1 = new Thread(()->multiChunk.findNewData());
        t1.start();
        t1.join();
        System.out.println("first part: ");
        System.out.println(multiChunk.streamDataList);

        Thread.sleep(1000);
        Thread t2 = new Thread(() -> multiChunk.findNewData());
        t2.start();
        t2.join();
        System.out.println("Second part");
        System.out.println(multiChunk.streamDataList);

        multiChunk.findNewData();
        multiChunk.transfer();
    }
...
//        double startTime = System.currentTimeMillis();
        ...
        if (dataset != null) {
            streamDataList.addAll(dataset.getFileList());
            lastDataSize = dataset.size();
        }else {
            System.out.println("No new files found");
        }

//        return dataset; // TODO return from thread. now it assigned to a global variable.

    }

    void contTransferData(XferList dataset) throws Exception {
...
//        String mDynamic = useDynamicScheduling ? "Dynamic" : "";
    ...
    private int[] allocateChannelsToChunks(List<Partition> chunks, final int channelCount) {
        ...
        */
                ...
                for (int i = 0; i < chunks.size(); i++) {
                    ...
                    Density densityOfChunk = chunks.get(i).getDensity();
    ...
    private void parseArguments(String[] arguments, AdaptiveGridFTPClient2 multiChunk) {
